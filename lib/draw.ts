...
export function isoSvg(brief: Brief, scheme: Scheme, night = false) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 560" font-family="ui-sans-serif,system-ui">
    <defs>
    ...
    <g transform="translate(250,210)">
      ...
      <text x="10" y="250" fill="${night ? "#f4efe4" : "#241c14"}" font-size="18">${escapeXml(scheme.name)}</text>
      <text x="10" y="274" fill="${night ? "#c9c2b3" : "#5a5044"}" font-size="13">${brief.widthFt} x ${brief.depthFt} ft · ${escapeXml(scheme.vibe)}</text>
    </g>
  </svg>`;
}
...
export function elevSvg(brief: Brief, scheme: Scheme) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 420" font-family="ui-sans-serif,system-ui">
    ...
    <text x="160" y="40" font-size="20" fill="#241c14">${escapeXml(scheme.name)} - yard elevation</text>
    <text x="160" y="64" font-size="13" fill="#5a5044">${brief.widthFt}' frontage shown · eave ${brief.heightFt}'</text>
...
export function planSvg(brief: Brief, scheme: Scheme) {
  ...
  const rects = zones
    .map((z, i) => {
      const h = Math.max(48, (H - 40) / zones.length);
      const block = `<g>
        <rect x="16" y="${y}" width="${W - 32}" height="${h - 8}" fill="${fills[i % fills.length]}" stroke="#2b241c"/>
        <text x="28" y="${y + 28}" font-size="16">${escapeXml(z.zone)}</text>
        <text x="28" y="${y + 50}" font-size="12" fill="#5a5044">${escapeXml(z.size)} · ${escapeXml(z.note)}</text>
      </g>`;
      y += h;
      return block;
    })
    .join("");
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 520" font-family="ui-sans-serif,system-ui">
    <rect width="900" height="520" fill="#f7f1e6"/>
    <text x="20" y="28" font-size="18">${escapeXml(scheme.name)} - floor plan concept</text>
    <text x="20" y="50" font-size="12" fill="#5a5044">A-101 · ${brief.widthFt}' x ${brief.depthFt}' · diagrammatic</text>
    <g transform="translate(70,70)">${rects}</g>
    <text x="20" y="500" font-size="11" fill="#6a6156">Not a construction document. Opening sizes and bearing walls need a designer of record.</text>
  </svg>`;
}
...
export function siteSvg(brief: Brief) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 520" font-family="ui-sans-serif,system-ui">
    ...
    <text x="54" y="68" font-size="18">AS-101 site concept · ${escapeXml(brief.region)}</text>
    ...
    <text x="54" y="460" font-size="12">North is assumed up. Walk the lot. This is a conversation starter, not a survey.</text>
  </svg>`;
}
